...
class ContentProcessor private constructor(
    ...
    companion object {
        ...
        fun get(bookName: String, bookOrigin: String): ContentProcessor {
            val processorWr = processors[processorKey(bookName, bookOrigin)]
            var processor: ContentProcessor? = processorWr?.get()
            if (processor == null) {
                processor = ContentProcessor(bookName, bookOrigin)
                processors[processorKey(bookName, bookOrigin)] = WeakReference(processor)
            }
            return processor
        }

        private fun processorKey(bookName: String, bookOrigin: String): String {
            return "$bookName\u0000$bookOrigin"
        }
